...
coda* AllocaCoda(fun* funzioni,int (*compare) (int,int), dfun* dfunzioni  ){
...
void IncreaseKey2(coda *Q,void* pun,int priorita){	
	void* padre;
...
void IncreaseKey(coda *Q,void* key,int priorita){
...
void DecreaseKey2(coda *Q,void* pun,int priorita){
...
void DecreaseKey(coda *Q,void* key,int priorita){
	helem* val = HashSearch(Q->HashTab, key);
	void* pun;
	if (val != NULL){
		pun = val->k;
...
int Insert(coda *Q, void *x, int k){
	...
	if(HashSearch(Q->HashTab,x) == NULL){
		...
		newdato->nome = x;
		newdato->p = (Q->comparison == MajorCompare ? -(INT_MAX) : INT_MAX );
		Q->heapsize++;
